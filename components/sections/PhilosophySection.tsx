		...
		<SectionContainer id={id} background="white" padding="lg">
			<Animated animation="slideUp">
				<div className="mx-auto max-w-3xl text-center mb-12">
					...
					<Text className="text-medium-grey">{intro}</Text>
				</div>

			<ol className="mx-auto max-w-4xl space-y-8" aria-label="Effuse Labs philosophy principles">
				{items.map((item, index) => (
					<Animated key={item.title} animation="fadeIn" delay={0.05 * index}>
						<li className="relative flex gap-6 rounded-xl border border-black/10 bg-white p-6 shadow-sm">
							<div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-brand-gold text-off-black font-bold" aria-hidden>
								{item.num}
							</div>
							<div>
								<H3 className="mb-2">{item.title}</H3>
								<Text className="text-medium-grey">{item.copy}</Text>
							</div>
						</li>
					</Animated>
				))}
