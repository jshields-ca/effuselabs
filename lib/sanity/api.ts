...
const defaultHeroContent: Omit<HeroSectionContent, '_id' | '_type'> = {
  ...
  description:
    'We pour out innovative solutions that transform operational burdens into competitive strengths for small businesses.',
...
export const getHeroContent = unstable_cache(
  async (): Promise<Omit<HeroSectionContent, '_id' | '_type'>> => {
    try {
      const heroContent =
        await sanityClient.fetch<HeroSectionContent>(heroSectionQuery)
      if (heroContent) {
        return {
          ...
          primaryCtaText:
            heroContent.primaryCtaText || defaultHeroContent.primaryCtaText,
          secondaryCtaText:
            heroContent.secondaryCtaText || defaultHeroContent.secondaryCtaText,
          primaryCtaHref:
            heroContent.primaryCtaHref || defaultHeroContent.primaryCtaHref,
          secondaryCtaHref:
            heroContent.secondaryCtaHref || defaultHeroContent.secondaryCtaHref,
          backgroundSettings:
            heroContent.backgroundSettings ||
            defaultHeroContent.backgroundSettings,
        }
      }
      return defaultHeroContent
    } catch (error) {
      console.warn(
        'Failed to fetch hero content from Sanity CMS, using default content:',
        error
      )
      return defaultHeroContent
    }
...
export const getHeaderContent = unstable_cache(
  async (): Promise<HeaderContent> => {
    try {
      const header = await sanityClient.fetch<Partial<HeaderContent> | null>(
        headerQuery
      )
      ...
      return {
        ...
        navLinks:
          Array.isArray(header.navLinks) && header.navLinks.length > 0
      ...
      console.warn(
        'Failed to fetch header content from Sanity CMS, using default content:',
        error
      )
